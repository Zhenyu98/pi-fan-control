...
一个用于 Raspberry Pi 5 官方风扇的高级温控服务，使用 Zone MPC 让风扇策略提前规划、更可控，把风扇输出花在更该花的地方。

它不是等温度越过几个固定阈值后再分段调速，而是根据当前温度、PWM、负载和短期历史预测接下来的温度轨迹，然后选择一个尽量低的 PWM，把机器维持在目标温度区间内。

在本机这组 10 分钟 AB 测试里（见「实测结果」），你会得到的效果：

- 在平均温度持平（约 `57.9 C`）的前提下，Zone MPC 的平均转速更低、中位转速明显更低（中位 `3647` vs `5816` RPM）——同样的散热效果下平均更安静。
- 控制脚本的 CPU 和内存开销几乎可忽略（`600 s` 内约 `0.65 s` CPU）。
- 在目标区间内，它倾向用能守住区间的最低 PWM，预测轨迹够凉时直接停转。
- 它并不能消除瞬时冲高：突发负载下的峰值温度和对照组相当（`62.2` vs `61.7 C`），因为负载突变是外生的，无法从温度历史预测。
...
所以它可能比默认策略更早让风扇动起来，目的是按预测轨迹分配风扇输出，而不是为了更吵；但它无法完全避免突发负载下的瞬时冲高。
...
模型预测得准不准？在一组未参与拟合的随机压力测试数据上，单步预测对实测 SoC 温度的平均绝对误差约为 `0.66 C`。由于 MPC 要在一个预测时域上做规划，下图右侧也给出了 rollout 误差随预测步数的增长：单步约 `0.7 C`，到第 12 步（约提前 `24 s`）约 `1.9 C`。

![ARX2 预测能力：在未参与拟合的数据上，单步预测对实测温度的误差约 0.66 C，rollout 误差随时域平缓增长](docs/assets/model_prediction_accuracy.svg)

`0.66 C` 算大吗？它已经接近硬件本身的下限，并不说明模型差。树莓派的温度传感器分辨率只有约 `0.55 C`——日志里每个读数都落在 0.55 °C 的网格上——所以光是“四舍五入到最近的传感器刻度”平均就要付出约 `0.27 C` 误差。作对比：一个最朴素的“下一步=当前值”持平预测器在这组数据上的 MAE 同样是 `0.64 C`；而模型的误差几乎全部集中在 CPU 负载突变的那几步（突变步 `1.56 C` vs 平稳步 `0.62 C`）。换句话说，单步误差主要来自传感器量化和无法预测的负载突变，而不是模型偏差。模型真正的价值体现在多步 rollout，以及“改变 PWM 会让温度怎么走”的推理上——这是持平预测器做不到的。
...
- 一个快速、低成本的教育项目：在 `$10` 级的真实硬件上跑通一整套 ARX 系统辨识 + Zone MPC 闭环，配有留出集预测指标和可复现的 AB 脚本——对已经懂 MPC、想看它落到一个真实被控对象上的学者和学习者来说，是一个小而完整、可上手的案例。
...
不保证。在本机 AB 测试里，平均温度持平时 Zone MPC 的平均/中位转速都低于缩放后的官方阶梯，所以那一轮里同样的散热效果下平均更安静。但官方策略只要把风扇开得更猛就能更凉，而突发负载下 Zone MPC 的转速一样可能骤升。它的目标是把风扇输出花在预测轨迹真正需要的地方，而不是无条件地安静。
...
- **大任务前提前预冷，预存稳定裕度。** 目前控制器是“反应式 + 预测式”：只有在负载和温度开始上升后才起转，而且 MPC 的 rollout 假设“未来负载 = 当前负载”，因此无法为一个**还没开始**的任务提前降温。好在热模型本身已经把负载作为 rollout 输入，架构是现成的——缺的只是一条**未来负载预测**。只要把它喂进去（任务在开始前主动发出的“我要跑了”信号、对 cron / systemd timer 上已知计划任务的感知，或在现有 shadow 样本上训练的负载预测器），MPC 就能提前拉高 PWM，让机器带着额外裕度进入重负载，而不是等温度爬上来再补救。
- 在更多环境温度和负载组合下采集辨识数据。
- 增加可选 RPM-aware 模型验证。
